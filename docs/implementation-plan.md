...
# -------------------------------------------------------
# Tariff & Balance Accounting (Phase 2+)
# -------------------------------------------------------
# tariff:
#   operator_preset: "mts"             # mts | megafon | beeline | tele2 | custom
#   balance_ussd: "*100#"              # USSD-запрос баланса
#   balance_regex: "([\\d\\.,]+)\\s*(?:руб|р|USD|EUR)"
#   auto_check_on_error: true          # Авто-запрос баланса при сбое SMS / GPRS
#   check_interval: 24h                # Регулярная проверка баланса
#   min_balance_alert: 50.0            # Порог алерта низкого баланса
#   packages:
#     sms_limit: 500                   # SMS в месяц
#     data_limit_mb: 1000              # МБ в месяц
#     reset_day_of_month: 1            # День сброса квоты
...
## Контроль баланса, тарифов и автодиагностика модема (Phase 2+)

### 1. Канал здоровья и готовности модема к работе

Для явного информирования пользователя и Home Assistant создаётся единый канал здоровья модема:

**Топик:** `gsm2mqtt/modem/{id}/health`

```json
{
  "state": "ready",            // ready | degraded | not_ready | error
  ...
  "currency": "RUB",
  "sms_remaining": 412,
  "updated_at": "2026-09-22T11:35:00+07:00"
}
```

- `ready`: SIM разблокирована, сеть найдена, сигнал достаточный, баланс положительный.
- `degraded`: Модем работает, но есть предупреждения (низкий баланс, слабый сигнал < -100 dBm, исчерпание пакета SMS).
- `not_ready`: Нет регистрации в сети, SIM заблокирована (PIN/PUK required), модем на перезагрузке.
- `error`: Ошибка оборудования, сбой serial порта, зависание AT-интерфейса.

**Топик алертов для срочных уведомлений:** `gsm2mqtt/modem/{id}/alert`

### 2. Автоматическая диагностика при сбоях (SMS / Интернет)

Если отправка SMS завершается ошибкой (`+CMS ERROR`, таймаут, rejection) или срывается GPRS/HTTP сессия, GSM2MQTT **автоматически запускает конвейер самодиагностики**:

```mermaid
flowchart TD
    FAIL["Сбой отправки SMS или GPRS"] --> DIAG["Запуск автодиагностики"]
    
    DIAG --> S1["1. Проверка SIM (AT+CPIN?)"]
    S1 -->|Сбой| A1["Алерт: SIM-карта недоступна/заблокирована"]
    
    S1 -->|OK| S2["2. Проверка регистрации (AT+CREG?)"]
    S2 -->|Сбой| A2["Алерт: Потеряна сеть оператора / Roaming запрещен"]
    
    S2 -->|OK| S3["3. Проверка сигнала (AT+CSQ)"]
    S3 -->|BER высокий / сигнал слабый| A3["Алерт: Зашумленный или слабый GSM-сигнал"]
    
    S3 -->|OK| S4["4. Проверка баланса (USSD / SMS пресет)"]
    S4 -->|Баланс <= 0 или пакет пуст| A4["Алерт: Исчерпан баланс или пакет SMS/трафика"]
    S4 -->|Баланс OK| A5["Алерт: Сбой на стороне SMSC оператора (перегрузка)"]
    
    A1 --> PUB["Публикация отчёта в\nmodem/{id}/diagnostic и health"]
    A2 --> PUB
    A3 --> PUB
    A4 --> PUB
    A5 --> PUB
...
### 3. Пресеты операторов и динамические команды

GSM2MQTT включает встроенные пресеты команд проверки баланса и остатков пакетов для операторов:

| Оператор | USSD запрос | Шаблон парсинга баланса (Regex) | Команда остатка пакетов |
|:---|:---|:---|:---|
| **МТС** | `*100#` | `(?i)(?:баланс|balance)[:\s]*([\d\.,]+)` | `*100*1#` |
| **Мегафон** | `*100#` | `(?i)(?:баланс|balance)[:\s]*([\d\.,]+)` | `*558#` |
| **Билайн** | `*102#` | `(?i)(?:баланс|balance)[:\s]*([\d\.,]+)` | `*106#` |
| **Tele2** | `*105#` | `(?i)(?:баланс|balance)[:\s]*([\d\.,]+)` | `*107#` |
| **Custom** | Произвольный код | Настраиваемый Regex | Настраиваемый USSD/SMS |

**Динамическое управление:** возможность через топик `gsm2mqtt/modem/{id}/tariff/set_preset` или YAML задать кастомный USSD-запрос и регулярное выражение для извлечения чисел.

### 4. Учёт пакетов и лимитов (Тарифный менеджер)

- **Квоты**: отслеживание количества отправленных SMS за расчётный период (день / месяц) с автоматическим сбросом в заданный день.
- **Трафик**: счётчик байт переданных данных при использовании GPRS/HTTP.
- **Стоимость**: расчёт расходов при поштучной тарификации.
- **MQTT-топики учёта**:
  - `gsm2mqtt/modem/{id}/accounting/status` — текущий баланс, израсходовано / осталось SMS и МБ.
  - `gsm2mqtt/modem/{id}/accounting/alert` — предупреждение при остатке менее N рублей или N сообщений.
